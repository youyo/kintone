...
| **OAuth 2.0**（Remote MCP のみ） | リモート MCP サーバ上でユーザ個別認可 / マルチユーザ | OAuth クライアント登録（redirect_uri = MCP サーバの公開 https URL）+ サーバ側 callback フロー（M13 で実装予定） | リモート MCP の Web フローでユーザがブラウザ承認 → サーバが TokenStore に保存 |
...
| リモート MCP サーバ（`mcp serve --listen ...`） | OAuth（サーバホスト型 callback、M13 で実装予定） |
...
2. MCP ツール呼び出し時に kintone トークン未取得が判定された場合、サーバが authorize URL を返す
3. ユーザーがブラウザで kintone 認可 → サーバの `/oauth/kintone/callback` が code を受信
4. サーバが access_token を取得し、TokenStore に `Domain + OIDC sub + AuthType=oauth` キーで保存
5. 以降の MCP 呼び出しで `PrincipalAPIFactory` が自動的にトークンを取得して kintone API 呼び出し
...
upstream kintone への OAuth トークンは、サーバホスト型 OAuth callback フロー（M13 で実装予定）が
