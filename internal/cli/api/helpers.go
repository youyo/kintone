...
func defaultNewAPI(in LoaderInput) (serviceapi.API, error) {
	...
	// KINTONE_STORE_CACHE_BYPASS=1 のとき CachingAPI を skip
...
//
// per-request lazy resolution: 各 API コールごとに Container.CacheForDecorator() を呼び、
// Container 側の lazy init / 自動回復ロジックを通す。Container 不在のときは
// (nil, nil) を返して upstream にフォールバック（fail-open）。
func newCacheProvider(ctx context.Context) serviceapi.CacheProvider {
	return func() (store.CacheStore, error) {
		...
		if container == nil {
			// Container 未注入の経路（テスト等）では fail-open
			return nil, nil
		}
